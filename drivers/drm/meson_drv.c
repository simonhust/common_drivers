				    ...
				    void *data, struct drm_file *file_priv)
{
	...
	/* No LUT flags set - caller requested non-SDR HDR but gave no LUT data */
	if (!(arg->lut_flags & MESON_HDR_LUT_ALL))
		return -EINVAL;

	lut_param.lut_on = LUT_ON;

	if ((arg->lut_flags & MESON_HDR_LUT_EOTF) && arg->eotf_lut_ptr) {
		uint32_t __user *user_eotf = u64_to_user_ptr(arg->eotf_lut_ptr);
		int i;
		for (i = 0; i < HDR2_EOTF_LUT_SIZE; i++) {
			uint32_t val;
			if (copy_from_user(&val, &user_eotf[i], sizeof(val)))
				return -EFAULT;
			lut_param.eotf_lut[i] = val;
		}
	}
	...
	if ((arg->lut_flags & MESON_HDR_LUT_OETF) && arg->oetf_lut_ptr) {
		uint32_t __user *user_oetf = u64_to_user_ptr(arg->oetf_lut_ptr);
		int i;
		for (i = 0; i < HDR2_OETF_LUT_SIZE; i++) {
			uint32_t val;
			if (copy_from_user(&val, &user_oetf[i], sizeof(val)))
				return -EFAULT;
			lut_param.oetf_lut[i] = val;
		}
	}
